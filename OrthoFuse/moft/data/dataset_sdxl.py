...
def tokenize_prompt(tokenizers, prompt):
    ...
    # List[torch.Tensor[1, tokenizer.model_max_length]]
...
class ImageDataset(Dataset):
    ...
    def process_img(self, img):
        ...
        return input_img, torch.tensor([w, h])
...
class StyleDataset(Dataset):
    ...
    def __init__(
        ...
        self.instance_image_root = Path(instance_image_root)
        image_file = os.path.basename(self.instance_image_root)
        if not self.instance_image_root.exists():
            print("instance_image_root:", self.instance_image_root)
            raise ValueError("Instance images root doesn't exists.")

        self.json_file = Path(os.path.join(
            os.path.dirname(instance_image_root), 'data.json'
        ))
        if not self.json_file.exists():
            raise ValueError("Json data doesn't exists.")

        self.instance_images_path = [self.instance_image_root]
        self.num_instance_images = len(self.instance_images_path)

        with open(self.json_file, 'r') as file:
            data = json.load(file)

        instance_prompt = data[image_file][0] + ' in {0}'
        self.instance_prompt = instance_prompt.format(f'{placeholder_token} {class_name}')
